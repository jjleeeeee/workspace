...
- 콘텐츠 목적이 다른 Avatar type을 임의로 혼용하지 않아야 한다. (정체성 혼란)
...
- 동일한 정보 계층 안에서 Avatar size를 섞어 사용하지 않아야 한다. (레이아웃 흔들림)
...
- Birthday_Hat은 Circle 아바타 오른쪽 상단에 배치됩니다. 모든 사이즈에 노출됩니다.
...
| Size group                     | Extracted values                                  |
| ------------------------------ | ------------------------------------------------- |
| XXXXLarge, XXXLarge            | 32, 38, body-lg/system-700, headling-s/system-800 |
| XXLarge, XLarge, Large, Medium | 24, caption-m/system-400                          |
...
- 구현 시 `gradient/integrated-membership-gra` 같은 토큰 경로를 찾지 않고, 아래 stop과 transform 값을 직접 기준으로 합니다.
- `angle`은 `gradientTransform`에서 계산한 참고값이며, 정확한 재현 기준은 `gradientTransform`입니다.
...
| Angle              | `353.88deg`                                                                                  |
| Gradient transform | `[[0.7684494853, 0.5588471889, -0.1627408862], [-0.0824003071, 0.1141824573, 0.4778847098]]` |

| Stop | Position | Color variable                                  |
| ---- | -------- | ----------------------------------------------- |
...
| Type     | Size group                     | Extracted values                                       |
| -------- | ------------------------------ | ------------------------------------------------------ |
| Squircle | XXXXLarge, XXXLarge            | 32, 40, Stroke 3, Stroke 3                             |
| Squircle | XXLarge, XLarge, Large, Medium | 24, 24, 24, 24, Stroke 3, Stroke 3, Stroke 3, Stroke 3 |

## Badge_Dot

- Badge_Dot은 아바타 오른쪽 상단에 배치되며, 사용하여 업데이트 상태를 표시합니다. 모든 사이즈에 노출됩니다. (Badge_Dot과는 별개)
...
| Type                           | Properties   | Description                                                             | Values                                                                            |
| ------------------------------ | ------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| -                              | Mode         | Default를 선택하면 OS테마 환경에 따라 변경되고, Fixed를 선택하면 OS 테마 환경에 상관없이 다크테마로 보여집니다. | Default, Fixed                                                                    |
| -                              | Size         | 타입별로 다양한 크기로 제공되며 레이아웃 구성 요소에 맞춰 사용하도록 합니다.                             | XXXXLarge, XXXLarge, XXLarge, XLarge, Large, Medium, Small, XSmall, XXSmall, Tiny |
| Circle                         | Emoji        | 모든 사이즈에 노출됩니다.                                                          | True, False                                                                       |
| Circle                         | Birthday_Hat | Birthday_Hat은 아바타 오른쪽 상단에 배치되며,                                         |                                                                                   |
| 생일 상태를 표시합니다. 모든 사이즈에 노출됩니다.   | True, False  |                                                                         |                                                                                   |
| Circle                         | Moment_Ring  | Ring Case를 선택합니다. 모든 사이즈에 노출됩니다.                                        | Default, Unread, Membership Only, Read                                            |
| Squircle                       | Badge_Dot    | Badge_Dot은 아바타 오른쪽 상단에 배치되며,                                            |                                                                                   |
| 업데이트 상태를 표시합니다. 모든 사이즈에 노출됩니다. | True, False  |                                                                         |                                                                                   |
| Squircle                       | Host         | 모든 사이즈에 노출됩니다.                                                          | True, False                                                                       |

...
| 항목 | 값 |
|---|---|
| 출력 폴더 | `../chord-design-system/tokens` |
| 출력 파일 | `color.json` |
| modes | `light`, `dark` |
...
```text
file: ../chord-design-system/tokens/color.json
collection: WDS_tokens
tokens: 408
modes: light, dark
base/color output: 0
...
└── validate-token-catalogs.mjs
```

실행 순서:

1. `node tools/extract-figma-color-tokens.test.mjs`
2. `node tools/extract-figma-color-tokens.mjs --raw-output outputs/raw/figma.variables.local.<YYYY-MM-DD>.json`
3. `node tools/validate-token-catalogs.mjs --input-dir ../chord-design-system/tokens`
...
6. `color.json`으로 저장
7. `tools/validate-token-catalogs.mjs`로 검증

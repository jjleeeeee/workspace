...
sources:
  color: ../00_tokens/tokens.color.v1.0.json           # 408 tokens, light/dark
  size: ../00_tokens/tokens.size.v1.0.json              # 66 tokens (radius, padding, margin, button-height)
  typography: ../00_tokens/tokens.typography.v1.0.json   # 71 tokens (default 3-platform + circular)
  icons: ./icons/                                        # SVG assets — 16px (270), 24px (342), 64px (25)
  components:
    button: ./components/button.md                       # 6 variants × 4 states, prop API, 3-platform code
    avatar: ./components/avatar.md                       # 11 sizes, Circle/Squircle, Ring/Badge/장식
    badge: ./components/badge.md                         # Badge_Dot (3 sizes) + Badge_Number (count/N)

# ──────────────────────────────────────────
# colors  — sRGB hex only (#RRGGBB)
#   SoT: tokens.color.v1.0.json (408 tokens)
...
#   SoT: tokens.color.v1.0.json (Gradient/* 항목)
...
#   SoT: tokens.typography.v1.0.json (71 tokens, 원자 단위)
...
#   SoT: tokens.size.v1.0.json → system/size/radius/box/*
...
#   SoT: tokens.size.v1.0.json → system/size/icon/*
...
#   SoT: tokens.size.v1.0.json → system/size/padding/box/*, margin/screen/*, button-height/*
#   아래는 curated 13개.
# ──────────────────────────────────────────
spacing:
...
#   SoT: tokens.size.v1.0.json → size shadow/* (base variables) + Figma 정의 페이지
...
#   Skipped variants (filled-black, outlined-gray 등): components/Button.md 참조.
# ──────────────────────────────────────────
components:
...
<!-- TODO: 페르소나 정의 (팬·아티스트). 여정 단계, 시각 위계 발현, 시스템 결정에 미치는 영향. -->

## Colors

> **SoT:** [tokens.color.v1.0.json](../00_tokens/tokens.color.v1.0.json) — 408 tokens (246 color + 162 fixed_color), light/dark 모드. 위 frontmatter는 curated 18개. curated에 없는 색이 필요하면 반드시 SoT JSON을 열어 확인할 것.
...
- **Button** — 버튼 컬러 토큰(`button-default` / `button-black` / `button-disabled` / `button-hover` / `button-ghost`)의 사용 규칙과 위계는 본 섹션에서 다루지 않는다. [Components/Button](./components/button.md) 참조.
...
> **SoT:** [tokens.color.v1.0.json](../00_tokens/tokens.color.v1.0.json) — `Gradient/*` 항목.
...
> **SoT:** [tokens.typography.v1.0.json](../00_tokens/tokens.typography.v1.0.json) — 71 원자 토큰 (fontSize / lineHeight / fontWeight 별도), ios/aos/web 3-platform 모드. 위 frontmatter는 생산 Figma 기준 조합 스타일 12개. 목록에 없는 스타일 조합이 필요하면 SoT JSON에서 원자 토큰을 조합할 것.
...
> **SoT:** [tokens.size.v1.0.json](../00_tokens/tokens.size.v1.0.json) — spacing 관련 토큰 (`padding/box` 18단계, `padding/thumb` 별도 패밀리, `margin/screen` 4단계). 위 frontmatter는 curated 13개. `padding-box-25` (2px), `padding-box-400/500/600/800`, 음수 padding 등 curated 외 토큰은 SoT 직접 참조.
...
> **SoT:** Shadow base variables는 [tokens.size.v1.0.json](../00_tokens/tokens.size.v1.0.json)의 `size shadow/*`. Dark surface 단계는 [tokens.color.v1.0.json](../00_tokens/tokens.color.v1.0.json)의 `surface/default-*`. 단계별 시각 spec은 Figma [Chord DS — Drop Shadow](https://www.figma.com/design/DWEduE6GfxYMlyxKPNJ8jA/Chord-Design-System?node-id=6956-40762) 참조. 위 frontmatter는 curated 6개.
...
> **SoT:** [tokens.size.v1.0.json](../00_tokens/tokens.size.v1.0.json) — radius 관련 토큰 (`radius/box` 9단계, `radius/thumb` 3단계). 위 frontmatter는 curated 8개. seek bar용 `box-25` (2px) 등 curated 외 토큰은 SoT 직접 참조.
...
**SoT 미등록 — 임시 대응:** 현재 SoT(`tokens.size.v1.0.json`)에 `pill` 토큰이 등록되어 있지 않아, 현장에서 `100px` 하드코딩으로 우회 중이다. WDS_tokens에 `radius/pill` 추가 후 일괄 토큰 호출로 전환할 것 (TODO).

### Avatar Shape — 별도 시스템

아바타는 `radius/box`도 `radius/thumb`도 아닌 **avatar 전용 모양 시스템**(Circle / Squircle)을 따른다. [components/avatar.md](./components/avatar.md) 참조.
...
> **상세 스펙:** [button.md](./components/button.md) · [avatar.md](./components/avatar.md) · [badge.md](./components/badge.md)

...
get_design_context가 반환하는 React+Tailwind JSX에서:
  - data-node-id / data-name 속성 → node id / name
  - <p> 텍스트 내용 → TEXT 노드 characters
  - CSS var(--system\/color\/X, #hex) 패턴 → fills[].boundVariables (VAR_NAME 형식)
  - flex-col / flex(row) Tailwind 클래스 → layoutMode VERTICAL / HORIZONTAL
  - gap-[Npx], p-[Npx] 등 → itemSpacing, padding

출력: {"nodes": [...]}  ←  design_md_harness.py 호환 형식

사용:
  python3 figma_mcp_adapter.py \\
...
def _extract_css_vars(class_str: str) -> list[dict]:
    ...
    fills = []
    # 예: var(--system\/color\/status\/danger-red,#fe5b58)
    for m in re.finditer(
        r"var\(--([^,)]+),\s*([^)]+)\)",
        class_str.replace("\\/", "/")
    ):
        raw_name = m.group(1).replace("\\", "").replace("--", "")
        # CSS var 이름에서 color/surface/... 패턴 추출
        # system\/color\/X → system/color/X
...
def _extract_layout(class_str: str) -> dict:
    layout: dict[str, Any] = {}
    ...
    # padding: p-[Npx], px-[Npx], py-[Npx], pt/pr/pb/pl-[Npx]
...
def _text_content(fragment: str) -> str:
    """JSX 조각에서 <p> 태그 텍스트 추출."""
    texts = re.findall(r"<p[^>]*>\s*\{([^}]+)\}\s*</p>", fragment)
    literal = re.findall(r"<p[^>]*>\s*([^{<][^<]*?)\s*</p>", fragment)
    parts = [t.strip() for t in literal if t.strip()]
...
def _build_nodes(jsx: str) -> list[dict]:
    ...
    for m in tag_pattern.finditer(jsx):
        ...
        layout = _extract_layout(class_str)
        node.update(layout)
    ...
    # data-node-id를 가진 div의 내부 <p> 를 찾아 TEXT 노드로 등록
...
def _load_keep_ids(component_spec_path: str) -> tuple[set[str], set[str]]:
    """component_spec.json에서 keep_ids와 exclude_ids 추출."""
...
def _filter_nodes(nodes: list[dict], keep_ids: set[str], exclude_ids: set[str]) -> list[dict]:
    """keep_ids에 해당하는 노드만 유지. exclude_ids는 제거."""
    if not keep_ids:
        return nodes
    return [n for n in nodes
            if n["id"] in keep_ids and n["id"] not in exclude_ids]
...
def main() -> None:
    ...
    parser.add_argument("--jsx-input", required=True, help=".tmp/design_context.jsx 경로")
    parser.add_argument("--keep-ids-json", default="", help="fixture/component_spec.json 경로")
    parser.add_argument("--output", required=True, help="fixture/figma_raw.json 출력 경로")
